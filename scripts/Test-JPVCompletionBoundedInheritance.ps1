...
foreach ($state in @('COMPLETED_RESULT','VERIFIED_BLOCKER','SINGLE_NECESSARY_FOUNDER_DECISION','HARD_TOOL_OR_AUTHORITY_BOUNDARY','EXPLICIT_FOUNDER_PAUSE')) {
...
$truthFiles = @('JPV-CLAIM-TRUTH-CONSUMER-INHERITANCE.json','operations/governance/claim-truth-consumer.mjs','tests/claim-truth-consumer.test.mjs','tests/disclosure-authorization-consumer.test.ts')
foreach ($relative in $truthFiles) {
    if (-not (Test-Path -LiteralPath (Join-Path $RepositoryRoot $relative) -PathType Leaf)) { throw "JPV_CLAIM_TRUTH_INHERITANCE_FAILURE: missing $relative" }
}
...
if ($merge.contract_version -ne '1.0.0') { throw 'JPV_GITHUB_MERGE_AUTHORITY_INHERITANCE_FAILURE: contract version drift' }
...
foreach ($class in @('DETERMINISTIC_TEST_OR_FIXTURE','LOW_RISK_INTERNAL_IMPLEMENTATION','GENERATED_VERIFIED_RECEIPT','POLICY_PERMITTED_DEPENDENCY_UPDATE','NON_AUTHORITY_DOCUMENTATION')) {
    if ($class -notin $merge.machine_eligible_classes) { throw "JPV_GITHUB_MERGE_AUTHORITY_INHERITANCE_FAILURE: missing machine class $class" }
}
foreach ($class in @('BRANCH_RULESET','SECURITY_TRUST_ROOT','FINANCIAL_AUTHORITY','LEGAL_COMMITMENT','HUMAN_RIGHTS_OR_PRIVACY','IRREVERSIBLE_RECORD_DELETION','VERIFICATION_WEAKENING','EMERGENCY_BYPASS')) {
    if ($class -notin $merge.human_required_classes) { throw "JPV_GITHUB_MERGE_AUTHORITY_INHERITANCE_FAILURE: missing high-risk class $class" }
}

[ordered]@{
    status='PASS'
    control_id='JPV-AUTHORITY-INHERITANCE-001'
    claim_truth_consumer='BOUND'
    disclosure_authorization_lifecycle='BOUND'
    github_merge_authority='BOUND'
    ruleset_activation_verified=$merge.ruleset_activation_verified
