...
function requireIncludes(file, terms) {
  ...
  for (const term of terms) {
    if (!content.includes(term)) failures.push(`${file} is missing required term: ${term}`);
  }
}
function requireRegex(file, regex, description) {
  const content = read(file);
  if (!regex.test(content)) failures.push(`${file} failed check: ${description}`);
}
...
for (const pattern of forbiddenPatterns) {
  if (pattern.test(people)) failures.push(`PEOPLE-PROTECTION.md contains prohibited weakening language matching: ${pattern}`);
}

for (const requiredDoc of ['docs/production-review-checklist.md','docs/policy-index.md','docs/enforcement-map.md']) {
  if (!exists(requiredDoc)) failures.push(`Missing ${requiredDoc}`);
}

const workflowRoot = path.join(root, '.github', 'workflows');
if (fs.existsSync(workflowRoot) && fs.readdirSync(workflowRoot, { recursive: true }).length > 0) {
  failures.push('Retired GitHub Actions workflow surface is present.');
}
